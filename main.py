...
# =========================================================================
# 🔻 از اینجا به بعد (از def recall_memory_click تا قبل از def show_scan_options_popup)
# را پاک کنید و کد جدید زیر را جایگزین کنید:
# =========================================================================
    def find_all_scan_files(self, is_2prop_mode=False):
        ...
        try:
            ...
            if root and os.path.exists(root):
                ...
                base_dirs.append(os.path.join(root, "Cornix Winner"))
                base_dirs.append(os.path.join(root, "Documents"))
                base_dirs.append(os.path.join(root, "Download"))
        except Exception:
            pass
        ...
        try:
            ...
            ext_dir = context.getExternalFilesDir(None)
            if ext_dir:
                p = ext_dir.getAbsolutePath()
                base_dirs.append(p)
                base_dirs.append(os.path.join(p, "Cornix Winner"))
        except Exception:
            pass

        if hasattr(self, 'user_data_dir') and self.user_data_dir:
            base_dirs.append(self.user_data_dir)
            base_dirs.append(os.path.join(self.user_data_dir, "Cornix Winner"))
        ...
        for base in base_dirs:
            ...
            for root_path, dirs, files in os.walk(base):
                if "Android/obb" in root_path or "DCIM/.thumbnails" in root_path:
                    continue
                for f in files:
