...
# فراخوانی کتابخانه‌های اصلی Kivy
try:
...
class TwoPropScreen(BoxLayout):
    ...
    def show_combined_recall_popup(self, *args):
        ...
        btn_recall_pic.bind(on_release=recall_pic_act)  # <--- این خط اصلاح شد
        btn_cancel.bind(on_release=popup.dismiss)
    ...
    def recall_picture_click(self, *args):
        ...
        def on_file_click(filename):
            if self.pic_delete_mode:
                ...
                try:
                    ...
                    print(e)
                populate_pic_file_list(self.current_search_query)
            else:
        ...
        def toggle_pic_delete_mode(*args):
            ...
            self.btn_pic_delet_toggle.btn_color = [0.1, 0.6, 0.2, 1] if self.delete_mode else [0.8, 0.2, 0.2, 1]
            populate_pic_file_list(self.current_search_query)
...
class MainApp(App):
    ...
    def on_start(self):
        try:
            ...
            if platform == 'android':
                ...
                # اگر مجوز دسترسی کامل به فایل‌ها داده نشده، صفحه تنظیمات را باز کن
    ...
    def get_storage_root(self, is_internal=True):
        try:
            ...
            if platform == 'android':
                from jnius import autoclass
                Environment = autoclass('android.os.Environment')
                # بازگرداندن دقیقاً صفحه اول حافظه اصلی گوشی
    ...
    def recall_jpeg_with_location(self, is_internal):
        ...
        def trigger_pic_search_keyboard(*args):
            ...
            p = VirtualKeyboardPopup(title="Type query", callback=lambda q: populate_after_search(q), default_text=self.current_pic_search_query)
            self.register_popup(p)
            p.open()
